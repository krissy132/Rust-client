...
impl GameState {
    ...
    pub fn is_over(&self) -> bool {
        ...
        Color::ALL.iter().all(|c| self.undeployed(*c).is_empty())
    }
...
/// Remove any colors that no longer have possible moves from `state.valid_colors`.
/// Matches Kotlin `GameRuleLogic.removeInvalidColors`.
pub fn remove_valid_colors(state: &mut GameState) {
    // Server version:
    // if (!hasValidColors()) return;
    // if (getPossibleMoves(this).isEmpty()) {
    //     removeActiveColor(); // removes current_color and advances
    //     removeInvalidColors(); // iterative call
    // }
    // Kotlin is explicitly tail-recursive, but max depth is 4 (4 colors).
    // We mimic it with single-step iteration until no color can move.
    if state.valid_colors.is_empty() {
        return;
    }
    if possible_moves(state).is_empty() {
        let current_color = state.current_color();
        state.remove_active_color(current_color);
        // Recursive call (safety: tail-recursive, max 4 calls)
...
pub fn possible_start_moves(state: &GameState) -> Vec<Move> {
    ...
    for variant in kind.variants() {
        ...
        let (max_dx, max_dy) = bbox(&variant.shape);
        // Top edge: y=0, x 0..=l-1-max_dx (covers top-left through top-right inclusive).
        for x in 0..=(l - 1 - max_dx) {
            let pos = Coordinates::new(x, 0);
            let _ = try_add_start(state, kind, color, rotation, is_flipped, pos, &mut out);
        }
        // Right edge: x = l-1-max_dx, y 1..l-1-max_dy (skip corners; top-right from top edge, bottom-right from bottom edge).
        for y in 1..(l - 1 - max_dy) {
            let pos = Coordinates::new(l - 1 - max_dx, y);
            let _ = try_add_start(state, kind, color, rotation, is_flipped, pos, &mut out);
        }
    // Bottom edge: y = l-1-max_dy, x 1..=l-1-max_dx (start at 1 to avoid bottom-left with left edge).
    for x in 1..=(l - 1 - max_dx) {
        let pos = Coordinates::new(x, l - 1 - max_dy);
        let _ = try_add_start(state, kind, color, rotation, is_flipped, pos, &mut out);
    }
    // Left edge: x=0, y 1..=l-1-max_dy (start at 1 to avoid top-left with top edge).
    for y in 1..=(l - 1 - max_dy) {
...
pub struct MoveChange {
    ...
    /// Previous `valid_colors` before `remove_valid_colors` modified it.
    pub prev_valid_colors: Vec<Color>,
}
...
impl Move {
    ...
    pub fn make_move(&self, state: &mut GameState) -> Result<MoveChange, BMM> {
        ...
        let prev_valid_colors = state.valid_colors.clone();
    ...
    pub fn unmake_move(state: &mut GameState, change: MoveChange) {
        ...
        // Restore valid_colors (remove_valid_colors may have removed colors).
        state.valid_colors = change.prev_valid_colors;
        // Restore turn.
