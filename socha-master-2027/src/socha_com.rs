...
/// Maximum number of bytes the receive buffer is allowed to hold without making parse
/// progress. A real server message (memento + 4×21 shapes) is comfortably under 16 KiB;
/// anything larger indicates the parser is stuck on malformed XML. We pick 1 MiB as a
/// generous upper bound — large enough for any legitimate state, small enough to prevent
/// unbounded memory growth on a malformed/corrupted stream.
const MAX_BUF_BYTES: usize = 1 << 18; // 256 KiB – reduced to make buffer‑overflow detection robust under load
...
impl ComHandler {
    ...
    fn try_read_new(&mut self) -> Result<(), ReceiveErr> {
        // Guard against unbounded buffer growth on a malformed/corrupted stream:
        // if the buffer is already above `MAX_BUF_BYTES` and no successful parse has
        // trimmed it, the parser is stuck — abort instead of consuming more memory.
        if self.buf.len() >= MAX_BUF_BYTES {
            return Err(ReceiveErr::FailedToBuildRoomMessage(format!(
                "receive buffer exceeded {MAX_BUF_BYTES} bytes without making parse progress"
            )));
        }
        ...
        match self.reader.read(&mut tmp) {
            Ok(n) => {
                if n == 0 {
                    // EOF — server closed the stream (or never sent anything after accept).
                    // Surface as a connection-closed error so blocking loops in
                    // `wait_for_and_rm_str` / `wait_for_com_message` don't spin forever.
                    return Err(ReceiveErr::ConnectionClosed(
    ...
    fn attempt_get_com_messages(&mut self) -> Result<Vec<ComMessage>, ReceiveErr> {
        ...
        if let Some(recv_com_msg) = self.get_com_msg_and_rm() {
            for recv_joined in &recv_com_msg.joined {
            ...
            for recv_left in &recv_com_msg.left {
            ...
            for recv_admin_prepared in &recv_com_msg.admin_prepared {
