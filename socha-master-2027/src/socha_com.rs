...
pub struct ComHandler {
    ...
    /// Temporary buffer for XML message formatting to avoid repeated allocation.
    msg_tmp_buf: String,
}
...
impl ComHandler {
    ...
    // Set non-blocking only after the <protocol> handshake to avoid WouldBlock on write_all.
        ...
        let mut com = ComHandler {
            ...
            msg_tmp_buf: String::with_capacity(4096),
        };
    ...
    pub fn peek_move_request(&mut self) -> bool {
    ...
    fn get_com_msg_and_rm(&mut self) -> Option<ReceivedComMessage> {
        ...
        // Reuse msg_tmp_buf for XML wrapping to avoid format! allocation
        self.msg_tmp_buf.clear();
        self.msg_tmp_buf.push_str("<comMessage>");
        self.msg_tmp_buf.push_str(&self.buf);
        self.msg_tmp_buf.push_str("</comMessage>");
        let rs_msg = ReceivedComMessage::from_str(&self.msg_tmp_buf);
    ...
    fn try_read_new(&mut self) -> Result<(), ReceiveErr> {
        ...
        match self.reader.read(&mut tmp) {
            Ok(n) => {
                ...
                // XML protocol is guaranteed ASCII/UTF-8, so from_utf8 is safe and faster than from_utf8_lossy
                let chunk = std::str::from_utf8(&tmp[..n]).unwrap_or_else(|e| {
                    // Only fallback on error to keep consistency with original error path
                    log::error!("invalid UTF-8 in TCP stream: {}", e);
                    // Return empty string to avoid panic, but log the error
                    ""
                });
