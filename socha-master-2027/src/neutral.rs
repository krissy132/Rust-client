...
impl PieceShape {
    ...
    ///
    /// The dedup comparison compares shapes as **sorted** coordinate sets so that two
    /// (rotation, flip) pairs producing identical cell coverage via different iteration
    /// orders are correctly deduplicated.
    pub fn variants(self) -> Vec<ShapeVariant> {
        ...
        for r in rotations {
            for &flip in &[false, true] {
                let mut shape = self.transform(r, flip);
                // Sort coordinates canonically so dedup is order-insensitive.
                shape.sort();
