...
/// Mirrors `GameRuleLogic.removeInvalidColors` — when true, `make_move` removes colors without moves.
pub const REMOVE_INVALID_COLORS: bool = true;
...
impl GameState {
    ...
    pub fn is_over(&self) -> bool {
        self.valid_colors.is_empty() || self.round() >= ROUND_LIMIT
    }
...
pub fn possible_start_moves(state: &GameState) -> Vec<Move> {
    ...
    // Bottom edge: y = l-1-max_dy, x 1..=l-1-max_dx (start at 1 to avoid duplicate with top edge).
    for x in 1..=(l - 1 - max_dx) {
        let pos = Coordinates::new(x, l - 1 - max_dy);
        let _ = try_add_start(state, kind, color, rotation, is_flipped, pos, &mut out);
    }
    // Left edge: x=0, y 1..=l-1-max_dy (start at 1 to avoid duplicate with right edge).
...
impl Move {
    ...
    // Remove any color that can no longer move (no moves possible).
    if REMOVE_INVALID_COLORS {
        remove_valid_colors(state);
    }
