...
pub struct GameState {
    ...
    /// Bitboard per colour (compressed occupancy) – each colour maps to a 7‑word u64 array (448 bits, enough for 20×20).
    pub color_bitboards: HashMap<Color, [u64; 7]>,
    /// Combined occupancy bitboard (any colour). Same layout as above.
    pub occupied_bitboard: [u64; 7],
}
...
pub fn remove_active_color(&mut self, color: Color) -> bool {
    ...
    impl GameState {
        /// Set the occupancy bit for `color` at board index `idx` (0‑based).
        fn set_bit(&mut self, color: Color, idx: usize) {
            let word = idx / 64;
            let bit = idx % 64;
            if let Some(bb) = self.color_bitboards.get_mut(&color) {
                bb[word] |= 1u64 << bit;
            }
            self.occupied_bitboard[word] |= 1u64 << bit;
        }

        /// Clear the occupancy bit for `color` at index `idx`.
        fn clear_bit(&mut self, color: Color, idx: usize) {
            let word = idx / 64;
            let bit = idx % 64;
            if let Some(bb) = self.color_bitboards.get_mut(&color) {
                bb[word] &= !(1u64 << bit);
...
///
/// Implementation note: the four edge-by-edge enumeration loops overlap at the four
/// board corners, so we collect into a `HashSet` (rather than a `Vec`) to silently
/// discard those corner duplicates. This avoids the same `(rotation, flip, position)`
/// triple being emitted twice.
pub fn possible_start_moves(state: &GameState) -> Vec<Move> {
    ...
    let l = BOARD_LENGTH as i32;
    let mut out: HashSet<Move> = HashSet::new();
                ...
                for c in &coords {
                    if Board::contains(c.x, c.y) {
                        ...
                        *slot = color.to_field_content();
                        set_cells.push(*c);
                        // Update bitboards
                        let idx = (c.y as usize) * BOARD_LENGTH + (c.x as usize);
                        state.set_bit(color, idx);
                    }
                }
        ...
        for c in &change.set_cells {
            if Board::contains(c.x, c.y) {
                *state.board.get_mut(c.x as usize, c.y as usize) = FieldContent::Empty;
                // Update bitboards
                let idx = (c.y as usize) * BOARD_LENGTH + (c.x as usize);
                state.clear_bit(change.color, idx);
            }
        }
